...
export default function App() {
  ...
  // `strict: false` lets the same component serve both routes.
  const params = useParams({ strict: false }) as { playlistId?: string }
  const routePlaylistId = params.playlistId ?? null
  ...
  // --- Mount: Load playlist from the /p/:id route param ---
  const mountedRef = useRef(false)
  useEffect(() => {
    if (mountedRef.current) return
    mountedRef.current = true

    const urlPlaylistId = routePlaylistId
